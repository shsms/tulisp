...
impl std::fmt::Display for TulispObject {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{}", self.rc.read().expect("RwLock poisoned")))
    }
}
...
macro_rules! predicate_fn {
    ($visibility: vis, $name: ident $(, $doc: literal)?) => {
        ...
        $visibility fn $name(&self) -> bool {
            self.rc.read().expect("RwLock poisoned").$name()
        }
    };
}
...
impl TulispObject {
    ...
    pub fn base_iter(&self) -> cons::BaseIter {
        self.rc.read().expect("RwLock poisoned").base_iter()
    }
    ...
    pub fn fmt_string(&self) -> String {
        self.rc.read().expect("RwLock poisoned").fmt_string()
    }
...
impl TulispObject {
    ...
    pub(crate) fn set_unchecked(&self, to_set: TulispObject) {
        self.rc
            .write()
            .expect("RwLock poisoned")
            .set_unchecked(to_set)
    }

    pub(crate) fn set_global(&self, to_set: TulispObject) -> Result<(), Error> {
        self.rc.write().expect("RwLock poisoned").set_global(to_set)
    }
    ...
    pub(crate) fn addr_as_usize(&self) -> usize {
        Arc::<RwLock<TulispValue>>::as_ptr(&self.rc) as usize
    }
    ...
    pub(crate) fn assign(&self, vv: TulispValue) {
        *self.rc.write().expect("RwLock poisoned") = vv
    }

    pub(crate) fn clone_inner(&self) -> TulispValue {
        self.rc.read().expect("RwLock poisoned").clone()
    }

    pub(crate) fn inner_ref(&self) -> RwLockReadGuard<'_, TulispValue> {
        self.rc.read().expect("RwLock poisoned")
    }

    pub(crate) fn as_list_cons(&self) -> Option<Cons> {
        self.rc.read().expect("RwLock poisoned").as_list_cons()
    }

    pub(crate) fn ctxobj(&self) -> Option<TulispObject> {
        self.rc.read().expect("RwLock poisoned").ctxobj()
    }
    ...
    pub(crate) fn with_ctxobj(&self, in_ctxobj: Option<TulispObject>) -> Self {
        self.rc
            .write()
            .expect("RwLock poisoned")
            .with_ctxobj(in_ctxobj);
        self.clone()
    }

    pub(crate) fn with_span(&self, in_span: Option<Span>) -> Self {
        *self.span.write().expect("RwLock poisoned") = in_span;
        self.clone()
    }
    pub(crate) fn take(&self) -> TulispValue {
        self.rc.write().expect("RwLock poisoned").take()
    }

    #[doc(hidden)]
    pub fn span(&self) -> Option<Span> {
        self.span.read().expect("RwLock poisoned").clone()
    }
...
impl TryFrom<TulispObject> for f64 {
    ...
    fn try_from(value: TulispObject) -> Result<Self, Self::Error> {
        let res = value.rc.read().expect("RwLock poisoned").try_float();
...
impl TryFrom<TulispObject> for i64 {
    ...
    fn try_from(value: TulispObject) -> Result<Self, Self::Error> {
        let res = value.rc.read().expect("RwLock poisoned").as_int();
        res.map_err(|e| e.with_trace(value))
    }
}
